...
public class ProductDaoImpl implements ProductDao{
	...
	static {
		...
		try {
			...
			insprod=conn.prepareStatement("insert into products values(?,?,?,?,?,?)");
			delprod=conn.prepareStatement("delete from products where pid=?");
			getByID=conn.prepareStatement("select * from products where pid=?");
			updateprod=conn.prepareStatement("update products set name=?,qty=?,price=?,expirydate=?,cid=? where pid=?");
